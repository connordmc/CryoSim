...
* **Lumped Boundary Resistors:** Resistor plates inject $Q = I^2 R / n$ at their node (per-strand joints in parallel) and contribute the joints' lumped heat capacity (`heatCapacityJK` per joint) to the node's thermal inertia, so the heating transient is physical rather than mesh-dependent. Setting `coolingCapacityWatts > 0` heat-sinks the joint to the fridge with the same linearized $Q_{max} \tanh(T/4.2)$ curve as dynamic plates — modeling a load mounted on a cooled stage (e.g. the 1 Ω load bolted to the mixing chamber) instead of a joint floating on the wire end.
